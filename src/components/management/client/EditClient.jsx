...
const EditClient = ({ isEditOpen, closeEditClient, editClientData }) => {
  ...
  const {
    ...
    initialValues: {
      ...
      type: editClientData.type
        ? { label: editClientData.type, value: editClientData.type }
        : "",
      site: editClientData.site
        ? { label: editClientData.site, value: editClientData.site }
        : "",
      email: editClientData.email || "",
      phone: editClientData.phone || "",
      country: editClientData.country
        ? { label: editClientData.country, value: editClientData.country }
        : "",
      industry: editClientData.industry
        ? { label: editClientData.industry, value: editClientData.industry }
        : "",
      company_name: editClientData.company_name || "",
      company_address: editClientData.company_address || "",
      contact_person: editClientData.contact_person || "",
      opportunity_source: editClientData.opportunity_source
        ? {
            label: editClientData.opportunity_source,
            value: editClientData.opportunity_source,
          }
        : "",
      status:
        editClientData?.status === 1
          ? { label: "Active", value: 1 }
          : { label: "Deactive", value: 0 },
    },
    ...
    onSubmit: async (values) => {
      ...
      try {
        const res = await Axios.put(
          `client/update/${editClientData.id}`,
          payload
        );
        if (res.status && res.status >= 200 && res.status < 300) {
          setLoading(false);
          closeEditClient(false);
          //dispatch send reaquest to redus store
          dispatch(refresh());
